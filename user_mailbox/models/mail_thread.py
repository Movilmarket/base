...
class MailThread(models.AbstractModel):
    """Overwrite message_post method for private followers."""
    ...
    def message_post(self, body='', subject=None,
        ...
        """Overridden Message Post Method.
        ...
        :param int parent_id: handle reply to a previous message by
            adding the parent partners to the message in case of
            private discussion
        ...
        if self.ids and not self.ensure_one():
            raise exceptions.Warning(
                _('Invalid record set: should be called as model \
                    (without records) or on single-record recordset'))
        ...
        if self.ids:
            ...
            if model and model != self._name and \
                    hasattr(self.env[model], 'message_post'):
                return self.env[model].browse(self.ids).message_post(
                    ...
                    model_description=model_description, **kwargs)
        ...
        # 2: Private message: add recipients (recipients and author of
        # parent message) - current author
        ...
        if self._mail_flat_thread and model and not parent_id and self.ids:
            messages = mail_message.search(
                ['&', ('res_id', '=', self.ids[0]),
                 ('model', '=', model)], order="id ASC", limit=1)
        ...
        # ancestor, to avoid having more than 1 level of thread
            ...
            messages = mail_message.sudo().search(
        ...
        # 3. Attachments
        #   - HACK TDE FIXME: Chatter: attachments linked to
        #    the document (not done JS-side), load the message
        ...
        # Post the message
        # canned_response_ids are added by js to be used by
        # other computations (odoobot)
        ...
        new_message = mail_message.create(values)
